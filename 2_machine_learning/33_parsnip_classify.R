...
# # Decision tree classifier with parsnip

# This example demonstrates a simple classification
# task using the
# [decision_tree()](https://tidymodels.github.io/parsnip/reference/decision_tree.html)
# function in the 
...
model <- decision_tree(mode="classification") %>%
  set_engine("rpart") %>%
  fit(piece ~ base_diameter + height, data = chess_train)
...
test_results %>% 
  metrics(truth = piece, estimate = .pred_class) 
